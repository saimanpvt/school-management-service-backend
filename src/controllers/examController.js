...
exports.getExamRecord = asyncHandler(async (req, res) => {
  ...
  // 2. TEACHER
    ...
    if (!course || !course.teacherId || course.teacherId._id.toString() !== user._id.toString()) {
      return res.status(403).json({ success: false, message: "You don't own the course for this exam" });
    }
  }
  ...
  // 3. STUDENT
    ...
    if (!isClassMatch) {
      return res.status(403).json({ success: false, message: "Not allowed to view this exam" });
    }
  }
  ...
  // 4. PARENT
    ...
    if (!child) return res.status(403).json({ success: false, message: "Not allowed or No child in this class." });
  }
  ...
  const formattedExam = {
    ...
    course: course ? {
      ...
      teacherName: course.teacherId ? `${course.teacherId.firstName} ${course.teacherId.lastName}` : 'N/A'
    } : null,
...
exports.getExamsByCourse = asyncHandler(async (req, res) => {
  ...
  let courseQuery = isValidObjectId(courseId) ? { _id: courseId } : { courseCode: courseId };
  ...
  // TEACHER: Must own the course
    ...
    if (!courseDoc.teacherId || courseDoc.teacherId.toString() !== user._id.toString()) {
      return sendErrorResponse(res, HTTP_STATUS.FORBIDDEN, 'Access forbidden: You do not teach this course');
    }
  }
  ...
  // STUDENT: Must be enrolled in the Class linked to this Course
    ...
    if (!isEnrolled) {
      return sendErrorResponse(res, HTTP_STATUS.FORBIDDEN, 'Access forbidden: You are not enrolled in this course');
    }
  }
  ...
  // PARENT: Child must be enrolled in the Class linked to this Course
    ...
    const children = await Student.find({ parentId: user._id });
    if (!children.length) return sendErrorResponse(res, HTTP_STATUS.FORBIDDEN, 'No children found');
    ...
    if (!allChildClassIds.includes(courseDoc.classId.toString())) {
       return sendErrorResponse(res, HTTP_STATUS.FORBIDDEN, 'Access forbidden: No child enrolled in this course');
    }
  } 
  ...
  const exams = await Exam.find(examFilter)
    .sort({ examDate: 1, startTime: 1 }); // Sorted chronologically
  ...
  return sendSuccessResponse(res, HTTP_STATUS.OK, 'Exams retrieved successfully', formattedExams);
});